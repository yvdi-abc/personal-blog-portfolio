...
export default function GlobalToolbox() {
  ...
  const { heavyEffectsEnabled, toggleHeavyEffects } = useEffects();

  const activeTool = TOOL_REGISTRY.find(t => t.id === activeToolId);

  return (
    <div className="fixed bottom-6 left-6 z-[9999] flex flex-col items-start gap-3" style={{ cursor: 'auto' }}>
                ...
                <button
                  ...
                  className={`text-xs font-bold px-3 py-1.5 rounded-full transition-all ${
                    activeToolId === tool.id
                      ...
                      : 'bg-white/50 dark:bg-slate-700/50 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-600'
                  }`}
              ...
              {activeTool ? (
                ...
                >
                  <svg className="w-8 h-8 mb-2 opacity-50" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M11 4a2 2 0 114 0v1a1 1 0 001 1h3a1 1 0 011 1v3a1 1 0 01-1 1h-1a2 2 0 100 4h1a1 1 0 011 1v3a1 1 0 01-1 1h-3a1 1 0 01-1-1v-1a2 2 0 10-4 0v1a1 1 0 01-1 1H7a1 1 0 01-1-1v-3a1 1 0 00-1-1H4a2 2 0 110-4h1a1 1 0 001-1V7a1 1 0 00-1-1H4a1 1 0 01-1-1V4a1 1 0 011-1h3a1 1 0 001-1v-1z" />
                  </svg>
                  点击上方模块<br/>激活工具
                </motion.div>
              )}
      ...
      <button
        ...
        className={`w-12 h-12 rounded-full flex items-center justify-center shadow-xl backdrop-blur-xl border border-white/40 dark:border-white/10 transition-all duration-500 hover:scale-110 active:scale-95
          ${heavyEffectsEnabled ? 'bg-gradient-to-r from-purple-500 to-pink-500 text-white' : 'bg-white/70 dark:bg-slate-800/80 text-slate-700 dark:text-slate-400'}
        `}
        style={{ cursor: 'pointer' }}
        title={heavyEffectsEnabled ? '关闭全部特效' : '开启全部特效'}
      >
        {heavyEffectsEnabled ? (
      ...
      <button
        onClick={() => {
          setIsOpen(!isOpen);
          if (!isOpen && !activeToolId) setActiveToolId(TOOL_REGISTRY[0].id);
        }}
