...
export default function Comments() {
  ...
  useEffect(() => {
    // 检查 Gitalk 配置是否完整
    const configured = siteConfig.gitalkConfig.clientID &&
    ...
    if (!configured) {
      ...
      placeholder.innerHTML = `
        <p class="mb-2">💬 评论功能暂未配置</p>
        <p class="text-sm">请在 siteConfig.ts 中配置 Gitalk 评论系统</p>
      `;
    ...
    const loadGitalk = async () => {
      try {
        ...
        const gitalk = new Gitalk({
          ...
          id: (pathname.replace(/\/$/, '') || '/').substring(0, 50),
        ...
        console.error('Failed to load Gitalk:', error);
        if (containerRef.current) {
          containerRef.current.innerHTML = '<p class="text-center py-8 text-red-500">评论系统加载失败</p>';
        }
      }
    };
